...
import org.solinari.data.formater.*;
import org.solinari.server.driver.sina.*;
import org.solinari.stock.db.driver.*;
...
public class RealtimeCollector extends Thread {
	...
	private void write_base(String code, String[] datas, String row) throws IOException{
		if (!HBaseDriver.isTableExist(code)){
			HBaseDriver.createTable(code, stockFam);
		}
		
		if (!HBaseDriver.isRowExisting(code, row)){
			HBaseDriver.writeRow(code, row, stockFam, datas);
		}
